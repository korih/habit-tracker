...
export async function PATCH(req: NextRequest, { params }: { params: { id: string; date: string } }) {
...
export async function DELETE(req: NextRequest, { params }: { params: { id: string; date: string } }) {
  const user = await getCurrentUser(req)
  if (!user) return NextResponse.json({ error: 'Unauthorized' }, { status: 401 })
